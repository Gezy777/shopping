...
func (p ProductQuery) GetByName(productName string) (product Product, err error) {
	err = p.db.WithContext(p.ctx).Model(&Product{}).Where("name = ?", productName).Find(&product).Error
	return 
}
